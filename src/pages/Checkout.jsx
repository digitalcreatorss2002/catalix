...
const Checkout = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (step === 1) {
      ...
      const isPaid = paymentMethod === 'razorpay' || paymentMethod === 'card';
      ...
      const itemsForDb = cartItems.map(ci => ({
        order_id: orderId,
        product_id: ci.id,
        product_name: ci.name || ci.title || 'Product Item',
        product_image: Array.isArray(ci.images) && ci.images.length > 0 ? ci.images[0] : (ci.image || null),
        quantity: Number(ci.quantity || 1),
        unit_price: Number(ci.price || 0),
        total_price: Number(ci.price || 0) * Number(ci.quantity || 1),
      }));

      const finalizeOrder = async (payload, items) => {
        try {
        ...
        if (items.length > 0 && isSupabaseConfigured && supabase && supabase.from) {
          try {
            const dbItemsPayload = items.map(it => ({
            ...
            await supabase.from('order_items').insert(dbItemsPayload);
          } catch (e) {
            console.warn('order_items insert error:', e);
          }
        }

        // Backup in local demo storage for instant display
        const demoOrders = jsonStorage.get('demo_orders', []);
        jsonStorage.set('demo_orders', [{ ...payload, order_items: items }, ...demoOrders]);

        const demoItems = jsonStorage.get('demo_order_items', []);
        jsonStorage.set('demo_order_items', [...demoItems, ...items]);
        ...
        try {
          ...
          await initializeRazorpay(
            ...
            async (paymentResponse) => {
              ...
              orderPayload.payment_id = paymentResponse.razorpay_payment_id || null;
              await finalizeOrder(orderPayload, itemsForDb);
            },
            (error) => {
